console.info('VTS admin.js loaded: v20260915-image-upload-fix');
import { supabase } from './supabase.js?v20260915-image-upload-fix';
...
import { ADMIN_EMAIL } from './config.js?v20260915-image-upload-fix';
...
async function uploadImage(file){
  ...
  // A successful Storage upload is the source of truth for the upload operation.
  // Do not block product creation on a second browser request to the public URL:
  // Supabase's public delivery layer can briefly lag behind the Storage write.
  const {data:uploaded,error:e}=await withTimeout(
  ...
  if(!uploaded?.path)throw new Error('Storage did not return the uploaded image path.');

  const {data}=supabase.storage.from('product-images').getPublicUrl(uploaded.path);
  if(!data?.publicUrl)throw new Error('Storage returned no public image URL.');

  // The URL can be stored immediately. The browser will load the public asset
  // when the product is rendered instead of making product creation depend on
  // a separate Image() request that may temporarily fail after an upload.
