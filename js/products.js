...
async function boot(){
  ...
  if(featured){const f=await getFeaturedProducts(); featured.innerHTML=f.length?f.map(card).join(''):'<div class="empty-state">No featured products have been published yet.</div>'; attachViewHandlers(featured)}
  if(catalogue){
